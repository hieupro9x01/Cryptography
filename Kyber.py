...
def main():
    # Chọn thông số cho Kyber (ở đây sử dụng params768 làm ví dụ)
    params = params768
    ...
    while True:
        ...
        if choice == '1':
            ...
            # Mã hóa
            pk_input = input("Enter the public key (hex): ")
            seed = os.urandom(32)
            pk = bytes.fromhex(pk_input)
            plaintext = input("Enter the plaintext message: ")
            ct, K = Enc(pk, seed, params)
            print("Ciphertext (ct):", ct.hex())
            print("Shared Secret (K):", K.hex())

        elif choice == '3':
            # Giải mã
            sk_input = input("Enter the secret key (hex): ")
            ct_input = input("Enter the ciphertext (hex): ")
            sk = bytes.fromhex(sk_input)
            ct = bytes.fromhex(ct_input)
            K = Dec(sk, ct, params)
            print("Decrypted Shared Secret (K):", K.hex())
